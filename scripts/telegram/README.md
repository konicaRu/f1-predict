...
выбрать режим (`raceweek` / `deadline` / `results` / `remind` / `autoresults` / `aiplayer` / `adminflush`) → запустить вручную. Проверить, что
...
Если workflow упал с ошибкой вида `... не найден в .env` — проверьте, что все четыре секрета
