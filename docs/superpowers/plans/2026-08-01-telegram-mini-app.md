...
**Статус на 2026-08-06: приостановлено на Step 2/3, ждём повторного теста пользователем.**
Ход сессии:
- Step 1 (BotFather Menu Button) — сделан.
- Step 2 (смоук: новый пользователь через инвайт-код) — **пройден на мобильном**, работает целиком
  (экран `/redeem` с именем из Telegram, вход по коду, повторное открытие сразу на Календаре).
- Step 3 (кнопка в напоминании) — по пути найден и исправлен реальный баг (см. ниже), но финальная
  проверка (открывает ли кнопка именно `/predict/<id>` нужной гонки, а не общий календарь) **не
  завершена** — при последней попытке на мобильном приложение открылось на Календаре вместо
  прогноза, и вскоре после этого зависание (см. ниже) началось происходить на обеих платформах.
- **Найдено и исправлено:** ссылка вида `t.me/<bot>?startapp=predict_<raceId>` (без короткого
  имени Mini App) открывается на мобильных, но давала `BOT_INVALID` на Telegram Desktop —
  ...
  BotFather (`/newapp`, короткое имя `predict`) и переходом на `t.me/che_f1_predict_bot/predict?startapp=...`
  — коммит `b9457ca` на этой ветке.
- **Найдено, НЕ решено:** после этого фикса на Telegram Desktop форма «Вступление в лигу» стала
  зависать на кнопке отправки (бесконечный спиннер, без ошибки) — тестовая живая регистрация
  участника (Марта) прошла нормально через обычный браузер в это же время, значит бэкенд был
  здоров. Независимая проверка `telegram-auth` Edge Function напрямую (curl-эквивалент) — отвечает
  быстро (~1.3с) и корректно. Проверка Microsoft Edge WebView2 Runtime на машине пользователя —
  установлен (151.0.4129.59), устаревший IE-фолбэк (известный отдельный баг tdesktop на Windows)
  исключён. Затем зависание стало воспроизводиться **и на мобильном тоже** (экран «Не удалось
  проверить доступ — проблема с сетью», retry не помогает), при включённом и вроде бы стабильном
  VPN. Повторная независимая проверка backend (REST API `races`) с машины разработчика — снова
  быстро и без ошибок. Вывод: проблема не в коде и не в бэкенде, а в сетевом пути именно из
  Telegram Mini App WebView (моб. и десктоп) до Supabase — это ровно тот открытый вопрос из
  **Task 1** («прокси ли Telegram трафик изнутри себя, в обход системного VPN, или нет —
  по-прежнему неизвестно»), теперь с более сильными косвенными уликами в его пользу. Диагностика
  дальше требует WebView Inspector внутри Telegram Desktop (доступен только пользователю).
  Пользователь решил попробовать позже — возможно, разовая сетевая флуктуация. **Продолжить с
  повторной проверки Step 2/3, когда пользователь вернётся.**

**Статус на 2026-08-07: root cause сетевого зависания найден и подтверждён, добавлена
устойчивость в коде, готово к повторному Step 2/3.**
- **Root cause найден через WebView Inspector в Telegram Desktop** (`Экспериментальные настройки →
  webview → Enable webview inspecting`, ПКМ на Mini App → «Проверить»). Во вкладке Network
  зависший `predictions?select=race_id` был в статусе Pending бесконечно; ранее `users?select=
  is_admin` дважды занял 10+ секунд. Разбивка Timing первого же запроса в сессии (до него не было
  других запросов к этому хосту — значит дело не в очереди/лимите соединений) показала: DNS
  Lookup 0.5ms, Initial connection 37ms, SSL 35ms, Waiting for server 135ms — всё быстро, но
  **Stalled: 10.02s** (для другого повторного запроса — 30.04s). То есть время съедается ДО начала
  подключения — классический признак задержки согласования прокси (WPAD), не сети/бэкенда как
  таковых.
- **Подтверждено экспериментально:** у пользователя в Windows было включено «Определять параметры
  прокси автоматически» (Параметры → Сеть и Интернет → Прокси-сервер). После выключения — те же
  самые запросы (`users`, `races`, `predictions`) прошли за 108-746мс, ни одного Pending. WPAD
  подтверждён как первопричина: WebView-подпроцесс Telegram, похоже, заново проходит обнаружение
  прокси на каждое соединение, независимо от основного процесса Telegram (который, видимо,
  кэширует результат один раз). Это тот же открытый вопрос из Task 1 («прокси ли Telegram трафик
  внутри WebView») — теперь с точным механизмом, а не только косвенными уликами.
- **Не решаемо централизованно** — нельзя попросить всех участников лиги выключить WPAD в Windows.
  Вместо этого добавлена устойчивость в коде на случай похожих сетевых задержек у любого участника:
  - `redeemInvite()` добавлена в `db.ts` (тот же паттерн `withRetry`, что уже применяется в
    `savePrediction`/`openRace`/остальных мутациях) — раньше `RedeemInvite.tsx` дёргал
    `supabase.rpc('redeem_invite', …)` напрямую без таймаута, из-за чего при сетевом зависании
    кнопка «Вступить» висела вечно (`busy=true` навсегда, без ошибки и без «Повторить»). Это и
    есть точный код-путь того самого зависшего экрана из сессии 2026-08-06.
  - `AuthContext.tsx`: вызов `telegram-auth` получил `timeout: 10000` (раньше не имел таймаута
    вовсе — зависание здесь блокировало бы вообще весь вход в Mini App, даже хуже симптома выше).
  - `tsc -b` чистый. Живой прогон в браузере через Playwright не удался технически (dev-сервер был
    недоступен по сети из соседних вызовов инструмента в этой сессии — среда, не код), поэтому
    поведение в реальном браузере/WebView не проверено автоматизированно, только код-ревью и
    типизация. Стоит проверить руками в следующий смоук.
- **Продолжить:** повторный сквозной смоук Step 2/3 теперь, когда сеть на машине пользователя
  стабильна (WPAD выключен как минимум для проверки) — форма «Вступление в лигу» и кнопка в
  напоминании должны отработать без зависаний.
