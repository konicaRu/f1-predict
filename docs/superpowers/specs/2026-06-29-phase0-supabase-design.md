...
> **Поправка (2026-06-30): пивот на cloud-direct.** Локальный Docker-стек Supabase на машине
> разработчика не заработал (Docker не тянет образы из реестра). Поэтому при реализации отказались
> от `supabase start` / `db reset` / pgTAP и пошли **напрямую против облачного проекта** через
> session pooler. Что изменилось против дизайна ниже: (1) воркфлоу W1–W3 заменён на применение
> миграций раннером `scripts/db/runner.js`; (2) pgTAP-тесты (§7) заменены на эквивалентные Node-тесты
> в `scripts/db/*.test.js` (формула, view, 5 RLS-сценариев) — критерий готовности тот же и **выполнен
> (7/7)**; (3) подключение — Session pooler (`...pooler.supabase.com:5432`), т.к. direct-хост
> `db.<ref>.supabase.co` не резолвится на free-тарифе; (4) миграции применены раннером напрямую, без
> `supabase db push` (в облаке нет записей `supabase_migrations.schema_migrations`). **Решения по схеме,
> RLS, валидации, формуле (D1–D5, §2–§6) остались в силе** — ниже актуально.
