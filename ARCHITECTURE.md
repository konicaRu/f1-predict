...
**Снимок этой карты в двух форматах** (не обновляется автоматически — актуализировать вручную
при заметных изменениях структуры, вместе с changelog ниже):
`docs/architecture-map.html` (визуально, для человека) и `docs/architecture-map.json`
(машиночитаемо, вкладывать в промпт свежему сабагенту перед новой фичей).
