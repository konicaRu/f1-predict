...
- `deadline()` шлёт баннер (`sendTelegramPhoto`, `lib.js`) как multipart-загрузку файла, а не
  ...
  его байты.
- **Известный риск (не решён, 3 случая — 2026-08-31, 2026-09-10, 2026-09-21):** scheduled-прогоны
  GitHub Actions иногда полностью пропадают на много часов сразу у нескольких воркфлоу
  репозитория (`keepalive.yml` и `telegram-notify.yml`), без объяснения на стороне GitHub (Actions
  permissions в порядке, github status — operational). Гипотеза «деградация без недавнего push»
  опровергнута (2026-09-21 повторилось при активных пушах незадолго до этого). Причина не найдена.
  Смягчается идемпотентным дизайном `raceweek()`/`ensureCurrentWeekOpen()` — самовосстанавливаются
  на следующем прогоне, полной потери напоминания не было. Детали — `MEMORY.md` § Открытые вопросы.
...
- **Дедлайн считается по дню недели, не по реальному времени квалификации:** `deadlineUtc()`
  ...
  (`update races set deadline_utc=...`) + временно двигать `aiplayer` в `telegram-notify.yml` на
  тот же день, что и новый дедлайн. Разовые ручные правки, не автоматизировано — см. `MEMORY.md`
  § Открытые вопросы (TODO вернуть cron обратно после Баку) и Лог сессий 2026-09-15/2026-09-21.
