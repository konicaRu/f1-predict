...
- Edge Functions (Deno, `supabase/functions/`):
  - `admin-notify` — принимает событие от pg_net-триггера ИЛИ напрямую от фронта (anon-ключ,
    ...
    Для `prediction` к тексту клеится шутка от Gemini (`joke.ts`), построенная ТОЛЬКО на прошлых
    отыгранных прогнозах участника — содержимое свежего прогноза в промпт не попадает структурно,
    иначе пересылка сообщения в общий чат до дедлайна ломала бы слепой прогноз (конституция).
...
- `telegram-notify.yml` — один workflow, режимы по cron: `raceweek`/`remind` (пн), `deadline`
  (ср×2/чт×2), `autoresults`+`results` (каждые 2ч), `adminflush` (10:05 МСК ежедневно — разгрузка
  ночной очереди admin-уведомлений). Полное расписание и разбор каждого режима — `README.md` §
  Telegram-уведомления. Открытие очередной гонки (`status: demo -> open`) больше не ручное —
  `ensureCurrentWeekOpen()` в `notify.js` вызывается на каждом запуске (`main()`, любой режим),
  сама находит гонку этой недели и открывает.
- На режимах `raceweek`/`deadline` дополнительно (best-effort, не рвёт основной режим при сбое)
  запускаются два подшага:
  - `checkDriverPool()`: подтягивает `drivers` из Jolpica, сверяет с составом ближайшей сессии
    OpenF1, расширяет `race_driver_pool` открытых гонок и шлёт уведомление в оба чата, если
  ...
  - `predict.js:main()` (GridBot, `scripts/ai-player`) — раньше жил на отдельном крон-слоте раз в
    неделю (`aiplayer`, чт до дедлайна); теперь у него своего слота нет, вызывается отсюда на
    каждом из 5 raceweek/deadline прогонов недели. Гейты внутри (дедлайн прошёл / уже поставил
    прогноз) делают повторные вызовы no-op — самовосстановление на случай пропажи конкретного
    крон-слота (живьём поймано 2026-09-23, см. Changelog). `aiplayer` остался в
    `workflow_dispatch` как ручной режим для тестов.
- `deadline()` шлёт баннер (`sendTelegramPhoto`, `lib.js`) как multipart-загрузку файла, а не
  ...
  его байты.
- **Известный риск (не решён, 4 случая — 2026-08-31, 2026-09-10, 2026-09-21, 2026-09-23):**
  scheduled-прогоны GitHub Actions периодически пропадают — либо массово на много часов сразу у
  нескольких воркфлоу (первые три случая), либо точечно (2026-09-23: из двух кронов на один и тот
  же час выжил только один — не `aiplayer`+`deadline`, а `adminflush`). Без объяснения на стороне
  GitHub (Actions permissions в порядке, github status — operational). Гипотеза «деградация без
  недавнего push» опровергнута. Причина не найдена. Смягчается идемпотентным дизайном
  `raceweek()`/`ensureCurrentWeekOpen()`/`checkDriverPool()`/`predict.js:main()` — все безопасно
  вызывать многократно, самовосстанавливаются на следующем подходящем прогоне. Детали —
...
- Тексты `raceweek()`/`deadline()` («Дедлайн — <день недели> HH:MM МСК», список «ещё не сделали»)
  ...
  неверный текст на гонке со сдвинутым дедлайном (см. ниже). Починено навсегда, не хак — работает
  одинаково для обычных и сдвинутых гонок.
- **Дедлайн считается по дню недели, не по реальному времени квалификации:** `deadlineUtc()`
  (`scripts/import/deadlines.js`) всегда берёт «ближайший четверг перед гонкой», в предположении
  что гонка — воскресенье (даёт фикс. 3-дневный запас). Если у гонки весь календарь уикенда
  сдвинут (напр. гонка в субботу вместо воскресенья — так у Azerbaijan GP round 15, 2026-09-26),
  формула молча даёт другой запас (для субботы — 2 дня), и дедлайн приходится править вручную
  (`update races set deadline_utc=...`). Раньше это ещё требовало временно двигать `aiplayer` на
  тот же день в `telegram-notify.yml` — с self-healing GridBot (см. выше) это больше не нужно, он
  подхватит любой день дедлайна сам на ближайшем raceweek/deadline прогоне. Правка `deadline_utc`
  по-прежнему ручная, не автоматизирована — см. `MEMORY.md` § Лог сессий 2026-09-15/2026-09-21.
- `scripts/import/import.js calendar` синхронизирует `races` с Jolpica по ключу `(season, round)`
  — используется и для реальных перестановок календаря сезона (напр. добавление гонки в середину
  сезона сдвигает нумерацию всех последующих раундов, см. Changelog 2026-09-21), не только для
  первичного импорта. `importResults()` (ручной режим, не в автоматике) берёт верхнюю границу
  раунда из БД (`max(round)`), а не хардкодом — раньше отставала при росте числа гонок в сезоне.
...
## Changelog
### 2026-09-23 (шутка-комментарий к прогнозу участника)
- Миграция `0027_prediction_jokes.sql` — таблица истории шуток (`user_id`+`race_id` уникальны,
  RLS без политик, гранты только service-role). Применена к боевой БД, гранты проверены.
- `supabase/functions/admin-notify/joke.ts` — промпты (отдельный для ветерана, отдельный для
  новичка без истории), расшифровка кодов пилотов, дедуп и вызов Gemini. `format.ts` — у события
  `prediction` необязательное поле `joke`, клеится тем же сообщением (админ пересылает в общий
  чат штатным «Переслать», без инлайн-кнопки: принимать `callback_query` сейчас некому —
  у бота нет ни вебхука, ни поллинга).
- Ключевое ограничение: шутка строится только на прошлых отыгранных гонках. Свежий прогноз в
  промпт не передаётся вообще — защита структурная, а не «попросили модель не болтать».
- Дедуп: пять прошлых шуток уходят в промпт как «так уже шутили», плюс мера Жаккара с порогом
  0.6 и одна перегенерация. На живом прогоне три шутки подряд дали похожесть 0.09 и 0.20.
- Два дефекта пойманы только на живом прогоне, оба исправлены: Gemini отвечает 503 при
  перегрузке (добавлены 3 попытки с бэкоффом — тут же сработало на двух таймаутах подряд) и
  выдумывает имена пилотов по трёхбуквенным кодам (назвала `LIN` Лэнсом Строллом вместо Арвида
  Линдблада — теперь в промпт идёт словарь код→имя и запрет угадывать).
- Деплой и заведение секрета `GEMINI_API_KEY` в Supabase — вручную (обе операции запрещены
  политикой агента). Смоук в проде: `{"sent":true}`, шутка про Pavel сгенерирована и сохранена.
### 2026-09-23 (GridBot стал самовосстанавливающимся; таймауты в ai-player)
- Живой инцидент: единственный крон-слот `aiplayer` (раз в неделю) пропал — 4-й случай пропажи
  scheduled-прогонов GitHub Actions (детали в разделе «Автоматика» выше). GridBot молча не
  участвовал в гонке, без второго шанса, в отличие от `deadline` (у него 4 слота в неделю —
  избыточность уже была).
- `predict.js:main()` теперь безопасно звать многократно (гейты «дедлайн прошёл» / «уже поставил
  прогноз» — no-op), вызывается из `notify.js` на каждом из 5 raceweek/deadline прогонов в
  неделю вместо одного отдельного крон-слота. Отдельный слот `aiplayer` в `telegram-notify.yml`
  убран, остался только как ручной `workflow_dispatch`-режим. Коммит `19ddaa2`.
- Заодно: `fetchJolpica`/`askGemini`/`sendTelegram` в `scripts/ai-player/lib.js` получили таймаут
  (`AbortController`, 15-30с) — живьём поймано, что `fetch()` без таймаута может зависнуть на
  много минут на флапающей сети вместо явной ошибки, а ретраи в `fetchJolpica` от такого зависания
  не спасают. Коммит `5618971`.
- Вручную поставлен прогноз GridBot и отправлено пропавшее напоминание для Azerbaijan Grand Prix
  (round 15) как стопгап, пока фикс не выкатился.

### 2026-09-21 (текст напоминаний по дню недели дедлайна; реальный перенос ГП Бахрейна в Малайзию)
- Тексты напоминаний считают день недели дедлайна из даты (`mskWeekday()`, `isDeadlineDayMsk()`),
  а не строкой «четверг» — постоянный фикс после того, как уже отправленный анонс раунда 15 ушёл
  со старым (неверным) текстом. Коммит `2facb49`.
- Третий подряд случай пропажи scheduled-прогонов GitHub Actions (см. `MEMORY.md` § Открытые
  вопросы) — гипотеза про push опровергнута.
- Реальная новость: ГП Бахрейна перенесён на трассу Сепанг в Малайзию (2-4 октября 2026, встык
  между Баку и Сингапуром) — синхронизировано через `import.js calendar`, календарь Jolpica уже
  обновлён. Round 16 = Bahrain Grand Prix in Malaysia, всё после сдвинулось на +1, Абу-Даби стал
  round 23. Дедлайн Баку (round 15) заново поправлен на среду после синка. Заодно починен
  устаревший хардкод `round<=22` в `importResults()`. Коммит `a5c9fb8`.

### 2026-09-15 (проверка «голосовал ли один» + разовый сдвиг дедлайна Баку на среду)
- Анализ по просьбе пользователя (без изменений кода/БД): ни в одной из 5 засчитанных гонок
  минимума в 1 предикторе не было (даже без учёта GridBot) — убирать из зачёта нечего. Детали —
  `MEMORY.md` § Лог сессий.
- Azerbaijan GP (round 15) — гонка в субботу вместо воскресенья, весь календарь уикенда на день
  раньше. `deadlineUtc()` даёт четверг (2 дня запаса вместо обычных 3) — вручную поправлено на
  среду 2026-09-23 20:00 UTC. `aiplayer` временно перевешен со среды на четверг в
  `telegram-notify.yml`, разовое исключение только для этой гонки. TODO вернуть — `MEMORY.md` §
  Открытые вопросы.
