...
### 2026-07-15 (2c закрыта+деплой; Фаза 3 спланирована)
...
- **Фаза 3 (витрина) СПЛАНИРОВАНА, impl НЕ начат.** Ветка `phase3`: спека
  (`docs/superpowers/specs/2026-07-14-phase3-showcase-design.md`) + план (`.../plans/2026-07-14-phase3-showcase.md`,
  6 задач). Решения: Зачёт+Результаты таблицами (drift-chart отложен), подход A (клиент), `standings.ts` чистая
  функция + тонкие обёртки `getScores`/`listUsers`/`listDrivers`. Тест — сид R21 + 3 фикстур-игрока → смоук → очистка.
  **Осталось: исполнить план (subagent-driven с конституцией) → смоук → merge.** После Фазы 3 — MVP (0–3) играбелен.
