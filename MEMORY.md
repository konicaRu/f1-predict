...
**ФАЗА 3 (витрина) реализована и смоук-протестирована на ветке `phase3`** — Зачёт+Результаты,
drift-chart отложен отдельной фазой. Осталось: финальное ревью ветки → merge → deploy.
...
- **НАЙДЕНО 2026-07-15 (смоук Фазы 3): `AuthContext.loadMembership` без `withRetry`.** При транзиентном
  сетевом сбое до Supabase тихо возвращает `data=null` → реальный участник ошибочно считается не-членом
  → редирект на `/redeem`. Лечится обновлением страницы, не блокирует. Фикс — см. `docs/plan.md` §15.
