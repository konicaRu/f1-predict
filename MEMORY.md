...
**⚠️ ИНЦИДЕНТ 2026-07-14: проект уснул** (keepalive ×2/нед оказался мал — Supabase хочет активность БД
каждый день, а scheduled-джобы GitHub ненадёжны). Фикс: keepalive переведён на **×2/день** (реальный SELECT).
Разбудить — вручную из дэшборда Supabase (Restore/Unpause; данные целы 90 дней). Проверить, что после
пуша нового keepalive расписание подхватилось (Actions → keepalive → зелёные ежедневные прогоны).
