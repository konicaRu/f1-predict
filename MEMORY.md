...
### 2026-07-16 (беклог: бэкап в Google Sheets — реализация на паузе, ждёт ручной шаг)
- Brainstorm → спека (`docs/superpowers/specs/2026-07-15-sheets-backup-design.md`) → план
  (`.../plans/2026-07-15-sheets-backup.md`, 4 задачи). Решения: свой cloud-direct Node-скрипт
  (`pg`+`googleapis`), 3 вкладки (Прогнозы/Результаты/Очки), таблицу создаёт пользователь вручную +
  шарит сервис-аккаунту, скрипт сам создаёт вкладки, полная перезапись каждый запуск, без автосинка.
- **Исполнено subagent-driven на ветке `sheets-export`, Task 1-3 из 4 готовы и приняты** (spec+quality
  ревью, с одним циклом фиксов на каждый): `scripts/export/lib.js` (подключение к БД + Sheets API-клиент),
  `scripts/export/export.js` (3 SQL-запроса → человекочитаемые строки → запись в Sheets; ревью нашло и
  починили тихий пропуск при отсутствии кода пилота — теперь `console.warn`), `scripts/export/README.md`
  (инструкция по Google Cloud Console; ревью дополнило двумя уточнениями: что делать на экранах
  grant-access при создании сервис-аккаунта, и troubleshooting на «не расшарил таблицу»).
- **Task 4 — ручной шаг пользователя, не начат.** Пользователь начал настройку в Google Cloud Console,
  упёрся в требование Google (не наше): с мая 2025 Google Cloud блокирует доступ без 2FA/MFA на аккаунте.
  Нужно включить 2-Step Verification в настройках аккаунта Google, затем продолжить с шага 1 README
  (создание проекта → Sheets API → сервис-аккаунт → ключ → таблица → `.env` → `npm run export`).
- Ветка `sheets-export` запушена в origin (`git save`), не влита. **Осталось:** пользователь заканчивает
  Task 4 (включает 2FA, проходит Google Cloud Console setup, запускает скрипт, подтверждает вывод) →
  финальное ревью ветки → merge → (деплоя не требует, это оффлайн cloud-direct скрипт, не фронтенд).
